 ...
 * A forma dos objetos é a mesma que o app original recebia do backend, então
 * trocar o mock pela API de verdade não muda nada nas telas.
 */
...
export const events: EventItem[] = seeds.map((item) => {
  ...
  return {
    ...
    locations: item.latitude ? [{ latitude: item.latitude, longitude: item.longitude }] : [],
    user_id: 1,
  };
});
